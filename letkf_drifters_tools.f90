...
SUBROUTINE read_drifters(infile,v4d_all)
  ...
  DO di=1,num_drifters
    DO ti=1,num_times
      read(fid,'(I16,6F16.6,I16)',IOSTAT=ios) dids, dlon, dlat, ddepth, dtemp, dsalt, dtime, ditime
      IF (ios==0) THEN
...
SUBROUTINE write_ensmspr_drifters(file,v4d_sub)
  ...
  DO k=1,num_times
    DO i=1,nid1
      CALL com_distll_1(v4d_sub(i,k,1,1),v4d_sub(i,k,1,2),v4dm(i,k,1),v4dm(i,k,2),dist)
      v4ds(i,k) = dist**2
      DO m=2,nbv
        CALL com_distll_1(v4d_sub(i,k,m,1),v4d_sub(i,k,m,2),v4dm(i,k,1),v4dm(i,k,2),dist)
