...
SUBROUTINE obs_local(ij,ilev,var_local,hdxf,rdiag,rloc,dep,nobsl,nobsl_dr,nobstotal)
  ...
  REAL(r_size),INTENT(IN) :: var_local(nid_obs+nid_dobs)
...
!
  ...
  WRITE(6,'(A)') 'Hello from letkf_drifters_local'
...
!       dist_zero_a = xrad
        ...
        if (.true.) then !(modulo(n,10000) .eq. 0) then
          !STEVE: DEBUG: print out the important values for a selection of obs
          WRITE(6,*), "=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=" 
          WRITE(6,*), "DEBUG letkf_local.f90..."
          WRITE(6,*), "n = ", n
          WRITE(6,*), "dist_min = ", dist_min
          WRITE(6,*), "sigma_obs0 = ", sigma_obs0
          WRITE(6,*), "minr = ", minr
          WRITE(6,*), "maxr = ", maxr
          WRITE(6,*), "maxdN = ", maxdN
          WRITE(6,*), "maxdS = ", maxdS
          ...
          WRITE(6,*), "sigma_b = ", sigma_b
          WRITE(6,*), "sigma_obs = ", sigma_obs
          WRITE(6,*), "dist_zero_a = ", dist_zero_a
          WRITE(6,*), "dist = ", dist
!         print *, "dist > dist_zero_a = ", dist > dist_zero_a
          WRITE(6,*), "=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=" 
        endif
...
!--------- End of Observation Culling ---------!
 ...
 IF ( DO_DRIFTERS ) THEN
  DO n=1,nn_dr
    ...
    olev = obslev_dr(nobs_use_dr(n))
    oelm = obselm_dr(nobs_use_dr(n))
    WRITE(6,*) "Finish defining olat,olon,olev,oelm...."
...
!   endif
    ...
    CASE(id_s_obs) !(OCEAN)
        iobs=5
    END SELECT
    WRITE(6,*) "iobs = ", iobs, "var_local(iobs) = ", var_local(iobs), "TINY(var_local) = ", TINY(var_local)
    IF(var_local(iobs) < TINY(var_local)) CYCLE 
    !STEVE: skip obs that are set to "0" impact on this model variable.
    ...
    ELSE
        dlev = 0.0d0
    END IF
    WRITE(6,*) "dlev = ", dlev, "dist_zerov = ", dist_zerov
    IF(dlev > dist_zerov) CYCLE

    !
    ! horizontal localization
    !
    WRITE(6,*) "horizontal localization...localization_method = ", localization_method
    horizontal_localization_dr : if (localization_method .eq. 1) then
        !STEVE: make horizontal localization latitude dependent
        ! STEVE: make sigma_obs a linear/lookup function of latitude
        WRITE(6,*) "Start doing horizontal localization..."
        dist_min = sigma_obs0 * (SQRT(10.0d0/3.0d0) * 2.0d0)
        minr = dist_min
        maxr = dist_zero
        maxdN = abs( (90.0d0)*(pi/180.0d0)*re)
...
SUBROUTINE obs_local_sub_dr(imin,imax,jmin,jmax,nn_dr,nobs_use_dr)
  INTEGER,INTENT(IN) :: imin,imax,jmin,jmax
