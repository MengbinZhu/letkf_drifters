...
!    END DO
  ...
  it=num_times ! LUYU: We just consider the last time index FOR NOW (**TBD**)
    ...
    DO i=1,nid1
      id = i + myrank * nid1 
      if (dodebug) WRITE(6,*) "id= ", drifter_ids(id) ! LUYU:: not "id" is id index NOT ID   
    ...
    DO n=1,nv4d
      ...
      CALL obs_local_drifters(id,it,var_local(nv3d+nv2d+n,:),mean4d(i,it,:),hdxf,rdiag,rloc,dep,nobsl,nobsl_dr,nobstotal)
